...
// 1.2.a) Unidad Funcional y/o Unidad Complementaria
const m_ufuncional = [
  { rango: [0, 9], valor: 800 },
  { rango: [10, 20], valor: 600 },
  { rango: [21, 40], valor: 500 },
  { rango: [41, Infinity], valor: 400 },
];
...
/**
 * Retorna el valor modular correspondiente a la cantidad funcional
 * @param {Number} funcional - La cantidad funcional a considerar
 * @returns {Number} - El valor modular correspondiente a la cantidad funcional
 */
function funcionalValorModular(funcional) {
  const moduloEncontrado = m_ufuncional.find((modulo) => {
    const [min, max] = modulo.rango;
    return funcional >= min && funcional <= max;
  });
  return moduloEncontrado ? moduloEncontrado.valor * multiplicador : 0;
}
...
function calcularTotal({ parcelas, ddjj, preferencial, funcional }) {
  const valorModularFuncional = funcionalValorModular(funcional);

  let total = ddjj * m_ddjj + funcional * funcionalValorModular(funcional);
...
function totalPreferencial(parcelas, ddjj, funcional) {
  const valor_modulo_funcional = funcionalValorModular(funcional);

  let total = ddjj * m_ddjj + funcional * valor_modulo_funcional;
  if (parcelas != 0) total += parcelasValorModular(parcelas) * parcelas;
...
function crearTablaResultados(valoresEntrada) {
  ...
  agregarFila("Unidad Funcional", funcional, valor_modulo_funcional, resultadosMensura);
